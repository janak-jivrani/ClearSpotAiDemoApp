package com.zw.clearspotaidemo.pages
...
import com.zw.clearspotaidemo.models.CameraStreamDetailVM
...
class CameraStreamDetailFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(
            ...
            onlyOneCamera: Boolean
        ): CameraStreamDetailFragment {
            val args = Bundle()
    ...
    private lateinit var svCamera: SurfaceView
    ...
    private fun initViewModel() {
        ...
        viewModel.availableLensListData.observe(viewLifecycleOwner) { availableLensList ->
            ...
            if (availableLensList.isEmpty()) {
                val childView =
                    mrgLensTypeLayout.findViewWithTag<View>(CameraVideoStreamSourceType.DEFAULT_CAMERA.toString())
                if (childView != null) {
    ...
    private fun updateCameraStream() {
        if (isNeedPreviewCamera) {
            svCamera.visibility = View.VISIBLE
        } else {
            svCamera.visibility = View.GONE
        }
    ...
    private val onScaleChangeListener =
        RadioGroup.OnCheckedChangeListener { _: RadioGroup?, checkedId: Int ->
            when (checkedId) {
                R.id.rb_center_crop -> {
